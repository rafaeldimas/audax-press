...
class Taxonomy
{
     ...
     */
    public function __construct( $name, $slug, $object_type, $suffix = 's' ) {
        $this->name        = $name;
        $this->slug        = $slug;
    ...
    protected function labels() {
        $default = array(
            'name'                       => sprintf( __( '%s%s', 'odin' ), $this->name, $this->suffix ),
            'singular_name'              => sprintf( __( '%s', 'odin' ), $this->name ),
            'add_or_remove_items'        => sprintf( __( 'Add or Remove %s%s', 'odin' ), $this->name, $this->suffix ),
            ...
            'menu_name'                  => sprintf( __( '%s%s', 'odin' ), $this->name, $this->suffix ),
            'add_new_item'               => sprintf( __( 'Add New %s', 'odin' ), $this->name ),
            'new_item_name'              => sprintf( __( 'New %s', 'odin' ), $this->name ),
            'all_items'                  => sprintf( __( 'All %s%s', 'odin' ), $this->name, $this->suffix ),
            'separate_items_with_commas' => sprintf( __( 'Separate %s%s with comma', 'odin' ), $this->name, $this->suffix ),
            'choose_from_most_used'      => sprintf( __( 'Choose from %s%s most used', 'odin' ), $this->name, $this->suffix )
        );
